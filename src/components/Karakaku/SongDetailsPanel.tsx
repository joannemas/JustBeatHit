...
export default function SongDetailsPanel({
    ...
    gameId?: string,
    mortSubite: boolean,
    setMortSubite: (value: boolean) => void
}) {
...
const handlePlayClick = async (e: React.MouseEvent) => {
  ...
  if (
    ...
    router.push(`/game/karakaku/${gameId}?mortSubite=${mortSubite ? "1" : "0"}`);
  }
};
  ...
  return (
    <div className={styles.drawer}>
      <img
        src={
          "coverFile" in song
            ? URL.createObjectURL(song.coverFile)
            : `https://fyuftckbjismoywarotn.supabase.co/storage/v1/object/public/song/${encodeURIComponent(
                `${song.singer} - ${song.title.replace(/'/g, "")}`
              )}/cover.jpg`
        }
        alt={song.title}
      ...
      <div className={styles.section}>
        ...
        <label className={styles.checkboxRow}>
          <input
            type="checkbox"
            checked={mortSubite}
            onChange={(e) => setMortSubite(e.target.checked)}
          />
      ...
      <div className={styles.gameBlock}>
        ...
        <div className={styles.statsRow}>
          <div className={styles.stat}>
            <div className={styles.label}>SCORE</div>
            <div className={styles.value}>
              {lastGame?.score?.toLocaleString() ?? "-"}
            </div>
          </div>
          <div className={styles.stat}>
            <div className={styles.label}>FAUTES</div>
            <div className={styles.value}>{lastGame?.mistakes ?? "-"}</div>
          </div>
          <div className={styles.stat}>
            <div className={styles.label}>VITESSE</div>
            <div className={styles.value}>
              {lastGame?.word_speed ? lastGame.word_speed + " MPM" : "-"}
            </div>
          </div>
        </div>
      </div>
      ...
      {showPremiumPopup && (
        <div className={styles.premiumPopup}>
          <div
            className={styles.premiumPopupContent}
            ref={popupRef}
            role="dialog"
            aria-modal="true"
          >
            ...
            <p>
              Jouer cette musique nécessite un abonnement Premium. Souscrivez à
              un abonnement pour débloquer cette fonctionnalité et bien plus
              encore !
            </p>
            <div className={styles.premiumPopupButtons}>
              <Link
                href="/options/subscription"
