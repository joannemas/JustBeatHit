...
export async function replayGame(gameId: string, mortSubite?: boolean): Promise<{ success: boolean, message?: string } | void> {
    let redirectData: Database["public"]["Tables"]["games"]["Row"][] | undefined
    try {
        const supabaseAdmin = createAdminClient();
    ...
    const mortSubiteParam = mortSubite ? '?mortSubite=1' : '';
    redirect(`/game/${redirectData[0].game_name}/${redirectData[0].id}${mortSubiteParam}`);
}